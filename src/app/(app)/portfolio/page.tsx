...
export default async function PortfolioPage() {
  ...
  return (
    <PortfolioPageContent
      ...
      totalArea={totalArea}
      avgAssemblyDays={avgAssemblyDays}
    />
  );
}

/** Inner component so useTranslations has a client boundary-free path */
function PortfolioPageContent({
  ...
  projects: Awaited<ReturnType<typeof listPortfolioProjects>>;
  totalProjects: number;
  totalModules: number;
  totalArea: number;
  avgAssemblyDays: number;
}) {
    ...
    <div className="min-h-screen bg-brand-bone-100">
      ...
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-brand-bone-300/60 bg-white/95 backdrop-blur-md px-4 md:px-8">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-brand-gray">
          <Link href="/#features" className="hover:text-brand-olive-700 transition-colors">{tNav("features")}</Link>
          <Link href="/portfolio" className="font-semibold text-brand-olive-700">{tNav("portfolio")}</Link>
          <Link href="/pricing" className="hover:text-brand-olive-700 transition-colors">{tNav("pricing")}</Link>
        </nav>
        <AuthNav />
      </header>
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-8 py-16 text-center">
        <p className="label-caps mb-3">{t("hero.label")}</p>
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-8 mb-12">
        <div className="flex flex-wrap justify-center gap-8 md:gap-16 py-6 border-y border-brand-bone-300/60">
          <StatTile n={String(totalProjects)} label={t("stats.featuredProjects")} />
          <StatTile n={String(totalModules)} label={t("stats.modulesDesigned")} />
          <StatTile n={`${totalArea} m²`} label={t("stats.totalArea")} />
          <StatTile n={t("stats.days", { days: avgAssemblyDays })} label={t("stats.avgAssembly")} />
        </div>
      </section>
