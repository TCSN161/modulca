---
id: "modulca-pricing"
title: "ModulCA Pricing & Account Tiers"
category: "modulca"
tags: ["modulca", "pricing", "tiers", "explorer", "premium", "architect", "constructor"]
sources: ["ModulCA Internal Documentation"]
difficulty: "beginner"
lastUpdated: "2026-04-16"
proOnly: false
relatedArticles: ["modulca-platform", "modulca-13-steps", "cost-estimation"]
---

# ModulCA Pricing & Account Tiers

## Account Tiers

### Explorer (Free)
- **Price**: €0
- **Includes**:
  - Full 13-step design flow
  - Neufert AI Consultant (basic queries)
  - 2D layout designer
  - 3D visualization
  - Basic knowledge base access
  - Save 1 project (with account)
- **Best for**: Exploring modular design, first-time users

### Premium (€19.99/month)
- **Price**: €19.99/month (€199/year — 2 months free)
- **Everything in Explorer, plus**:
  - 50 AI renders per month (HD)
  - All 6 technical drawing types + PDF export
  - Auto-tour & building permit tracker
  - Full knowledge base with pro articles
  - Product catalog with real pricing
  - 10 saved projects + sharable links
- **Best for**: Homeowners actively designing their home

### Architect (€49.99/month)
- **Price**: €49.99/month (€499/year — 2 months free)
- **Everything in Premium, plus**:
  - 200 AI renders per month (4K)
  - DWG/DXF export for CAD
  - Team collaboration & client dashboard
  - Unlimited projects
  - Priority support
- **Best for**: Architects and professionals using ModulCA for clients

### Constructor (€149.90/month)
- **Price**: €149.90/month (€1499/year — 2 months free)
- **Everything in Architect, plus**:
  - Unlimited AI renders (4K)
  - White-label branding
  - Full analytics & API access
  - Dedicated support
  - Up to 200 modules per project
- **Best for**: Construction companies and enterprise users

## Construction Costs (Romania 2024-2025)

| Component | Basic | Premium |
|-----------|-------|---------|
| Module (per m²) | €800-1,200 | €1,200-1,800 |
| Foundation (per m²) | €50-100 | €50-100 |
| Transport (per module) | €500-2,000 | €500-2,000 |
| Assembly (per m²) | €100-200 | €100-200 |
| **Total turnkey** | **€1,000-1,500/m²** | **€1,500-2,000/m²** |

## Example Project Costs

| Home | Modules | Area | Basic Cost | Premium Cost |
|------|---------|------|-----------|-------------|
| 1-Bedroom (5 modules) | 5 | 35m² usable | ~€50,000-65,000 | ~€70,000-90,000 |
| 2-Bedroom (8 modules) | 8 | 56m² usable | ~€75,000-100,000 | ~€105,000-140,000 |
| 3-Bedroom (11 modules) | 11 | 77m² usable | ~€100,000-135,000 | ~€140,000-185,000 |
| Family home (14 modules) | 14 | 98m² usable | ~€130,000-170,000 | ~€180,000-235,000 |

*Costs are indicative for Romania. Netherlands and other EU markets will have different pricing.*
